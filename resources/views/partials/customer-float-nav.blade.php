...
<nav class="smslord-float-nav" aria-label="Quick navigation">
    ...
    <button type="button" class="smslord-float-nav__item {{ request()->is('cworld*', 'usa2*', 'world-sv2*', 'world-sv3*', 'orders*') ? 'is-active' : '' }}" data-dash-open="numbers-sheet" aria-haspopup="dialog" aria-controls="dash-numbers-sheet">
        <span class="smslord-float-nav__icon"><i class="ti ti-device-mobile"></i></span>
        <span class="smslord-float-nav__label">Numbers</span>
    </button>
    @else
    <a href="{{ url('orders') }}" class="smslord-float-nav__item {{ request()->is('orders*') ? 'is-active' : '' }}">
        <span class="smslord-float-nav__icon"><i class="ti ti-device-mobile"></i></span>
        <span class="smslord-float-nav__label">Numbers</span>
    </a>
    @endif
    ...
    <a href="{{ route('wallet.transactions') }}" class="smslord-float-nav__item {{ request()->is('wallet-transactions') ? 'is-active' : '' }}">
        <span class="smslord-float-nav__icon"><i class="ti ti-file-invoice"></i></span>
