...
class EsimController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('duration') && (int) $request->input('duration') > 0) {
            $query['duration'] = (int) $request->input('duration');
        }
        ...
        $durations = $this->orders->durationsForDropdown($type, $query['country'] ?? null);
